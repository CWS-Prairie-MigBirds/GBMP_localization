...
Modified by Erica Alex 2026-02-10
...
Note: remove the "1-1" index from the filename before running this script
...
def get_recorder_list():
    ...
    """
    cols = ["GSA-A","GSA-B","GSA-C","GSA-D","GSA-E","GSA-F","GSA-G"]
    rows = ["1","2","3","4","5","6","7"]
...
def get_recording_path(
    ...
    if "resampled" in data_dir:
        ...
        if hour_minute == "any":
            recordings = list(Path(data_dir).glob(f"{recorder}/{recorder}_{date}/S{date}*.wav"))
        else:
            recordings = list(Path(data_dir).glob(f"{recorder}/{recorder}_{date}/S{date}T{hour_minute}*.wav"))
...
def get_all_times(time, max_minutes=15):
    """
    Get all possible recording minutes given a potential start time.

    Arguments:
        time: hour and minute string in the format 'HHMM'
        max_minutes: how many consecutive minutes to generate

    Returns:
        list of HHMM strings
    """
    start = datetime.datetime.strptime(time, "%H%M")
    return [
        (start + datetime.timedelta(minutes=i)).strftime("%H%M")
...
def get_overflows(
    recorder, date, data_dir=DATA_DIR, logfile_name=None, logging=False):
    ...
    """
    txtfiles = list(glob(f"{data_dir}/{recorder}*/{recorder}_{date}/*.txt"))
...
def get_loclog_contents(data_dir, recorder, date):
    ...
    """
    # List all logfiles
    date_logfiles = list(glob(f"{data_dir}/{recorder}/{recorder}_{date}/loclog.txt"))
    try:

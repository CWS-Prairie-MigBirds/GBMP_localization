...
#by Megan Edgar (megan.edgar@ec.gc.ca)
#2026-01-06
#https://github.com/megredgar/ValidationApp

#An R Shiny app for rapid visual and auditory validation of bird detections from HawkEars (or similar CNN/embedding-based recognizers).
#HawkEars Validator takes the model's output CSV — with filenames, start/end times, species, and scores — cuts 3-second audio snippets 
#from long ARU recordings, and serves them up with synchronized spectrograms so you can efficiently confirm, reject, or flag predictions as unsure.

#Adapting the methods of Tseng et al 2025 (https://doi.org/10.1007/s10336-025-02260-w), raw HawkEars scores (0.1-1.0) are binned and a random sample
#of detections from each bin are selected to be verified.
...
labels_file  <- "REDACTED" #csv with HawkEars detections
audio_dir    <- "REDACTED" #path where audio files are stored
results_file <- "REDACTED" #where to save verified detections

#create temporary directory for 23s audio clips
...
TSENG_MIN  <- 0.10  #minimum score
TSENG_MAX  <- 1.00  #maxmium score
TSENG_STEP <- 0.05  #bin width
TSENG_N    <- 10    #number to validate per bin
